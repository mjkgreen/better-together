...
import type { Event, User, EventAttendee } from "@prisma/client";

type AttendeeWithUser = EventAttendee & { user: User };
...
export default function Matchmaker({ attendees, event }: MatchmakerProps) {
  ...
  const findMatches = async () => {
    if (!selectedAttendeeId) {
      setError("Please select a user to find matches for.");
      return;
    }
    ...
    try {
      const response = await fetch("/api/match", {
        ...
        body: JSON.stringify({ userId: selectedAttendeeId, eventId: event.id }),
      });
  ...
  const selectedUser = attendees.find((a) => a.userId === selectedAttendeeId);
      ...
      <div className="w-full max-w-6xl mx-auto p-6 lg:p-8">
        ...
        <div className="bg-white shadow-xl rounded-2xl p-8 mb-8 border border-gray-100">
          <div className="flex flex-col lg:flex-row gap-8 items-start lg:items-end">
            <div className="flex-1">
              ...
              <select
                id="user-select"
                className="w-full p-4 border-2 border-gray-200 rounded-xl focus:ring-4 focus:ring-indigo-100 focus:border-indigo-500 transition-all duration-200 text-gray-700 font-medium"
              ...
              >
                ...
                {attendees.map((attendee) => (
                  <option key={attendee.userId} value={attendee.userId}>
                    {attendee.user.name} • {attendee.user.role} at {attendee.user.company}
                  </option>
                ))}
              </select>
            </div>
            <div className="w-full lg:w-auto">
              <button
                onClick={findMatches}
                disabled={isLoading || !selectedAttendeeId}
                className="w-full lg:w-auto bg-gradient-to-r from-indigo-600 to-purple-600 text-white font-bold py-4 px-8 rounded-xl hover:from-indigo-700 hover:to-purple-700 disabled:from-gray-400 disabled:to-gray-400 transition-all duration-200 ease-in-out shadow-lg hover:shadow-xl transform hover:scale-105 disabled:transform-none"
              >
                {isLoading ? (
          ...
          {selectedUser && (
            <div className="mt-8 p-6 bg-gradient-to-r from-indigo-50 to-purple-50 rounded-xl border border-indigo-100">
              <div className="flex items-start space-x-4">
                ...
                <div className="flex-1">
                  <h3 className="font-bold text-gray-800 text-lg">{selectedUser.user.name}</h3>
                  <p className="text-indigo-600 font-medium">
                    {selectedUser.user.role} at {selectedUser.user.company}
                  </p>
                  <div className="mt-3">
                    <h4 className="font-semibold text-gray-700 text-sm mb-1">Networking Goals:</h4>
                    <p className="text-gray-600 italic leading-relaxed">"{selectedUser.goals}"</p>
                  </div>
                  <div className="mt-2 flex flex-wrap gap-2">
                    {selectedUser.keywords?.split(",").map((keyword, idx) => (
                      <span
                        key={idx}
                        className="px-3 py-1 bg-white text-indigo-600 text-xs font-medium rounded-full border border-indigo-200"
                      >
                        {keyword.trim()}
                      </span>
                    ))}
        ...
        {isLoading && (
          <div className="text-center py-16">
            ...
            <p className="text-gray-500 mt-2">Our AI is finding your perfect matches</p>
          </div>
        )}
              ...
              {matches.map((match, index) => (
                ...
                >
                  <div className="flex items-start justify-between mb-4">
                    ...
                    <div className={`px-3 py-1 rounded-full text-xs font-bold ${getMatchScoreColor(match.matchScore)}`}>
                      {match.matchScore}%
                    </div>
                  </div>
